    ...
    s.tool('clean_compile_project', 'Performs one explicit application.clean() followed by one application.build(), then returns bounded same-call compiler evidence. Never saves the project.', {
        ...
        const script = scriptManager.prepareScriptWithHelpers('clean_compile_project', { PROJECT_FILE_PATH: escaped }, ['_text_utils', '_message_utils', 'ensure_project_open']);
        const result = await executor.executeScript(script, 900000);
        ...
        if (uniqueMarkers) {
            try {
                summary = JSON.parse(output.substring(start + startMarker.length, end).trim());
            }
            catch {
                summary = null;
            }
        ...
        const recordsValid = summary !== null && Array.isArray(summary.records) &&
            summary.records.every((record) => record !== null && typeof record === 'object' &&
                record.severity === 'warning' &&
                typeof record.text === 'string' && record.text.trim().length > 0 &&
                Buffer.byteLength(record.text, 'utf8') <= 4096) &&
            summary.records.reduce((total, record) => total + Buffer.byteLength(record.text, 'utf8'), 0) <= 262144;
        const diagnosticsValid = summary !== null &&
            Array.isArray(summary.diagnosticRows) &&
            summary.diagnosticRows.every((row) => typeof row === 'string' && row.trim().length > 0 &&
                Buffer.byteLength(row, 'utf8') <= 2048) &&
            summary.diagnosticRows.length <= 100 &&
            summary.diagnosticRows.reduce((total, row) => total + Buffer.byteLength(row, 'utf8'), 0) <= 65536;
        const clearResultsValid = summary !== null &&
            Array.isArray(summary.categoryClearResults) &&
            summary.categoryClearResults.length === 2 &&
            summary.categoryClearResults.every((item) => item !== null && typeof item === 'object' &&
        ...
        if (!contractValid) {
            return {
                content: [{ type: 'text', text: `Clean compile contract unavailable or rejected for ${escaped}.` }],
                isError: true,
            };
        }
        return {
