...
try {
  ...
  foreach ($relativeServer in @("src\server.ts", "dist\server.js")) {
    $fixtureServer = Join-Path $fixturePackage $relativeServer
    $transientBackup = "$fixtureServer.bak_pre_semantic_snapshot_contract"
  ...
  if (Test-Path -LiteralPath $typescriptModule) {
    $typescriptAsset = Join-Path $patchRoot "get_ctrlx_semantic_snapshot.tool.ts"
    & node -e @'
const fs = require('fs');
...
'@ $typescriptModule $typescriptAsset
    if (-not $?) { throw "Semantic TypeScript asset syntax check failed." }
  }
