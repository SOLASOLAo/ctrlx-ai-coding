...
└── <AiRepository>/           当前 AI 旁车仓库
    ...
    ├── generated/             可重建的流程计划/可选 I/O designator ASC，禁止手改
...
3. Stage 1 消费者只读生成 Git/指纹/ownership 审计报告；配置了 `sources.ioDesignators` 时，
   同时核对本工位完整 CSV 与 CpStudio 导出的 BusConfig，并由 Stage 2 将匹配哈希绑定进 action；
...
- `specs/io-designators.csv` 与 `generated/cpstudio-io-designators.asc`（可选）：本工位完整通道事实
  与确定性 CpStudio 导入文件；不得复制其他工位点位；
...
`Build` 校验并确定性重建计划及可选 ASC；`Check` 重算并逐字检查产物与源指纹。
